...
implemented two different ways.  Side by side, they show how the same
behaviour can be achieved with direct syscall table manipulation versus the
kernel livepatch mechanism.
...
| Forward to original | Saved function pointer (`orig_kill`) | `kill_pid_info(sig, info, pid)` |
| Restore on unload | Manual (`lkm_exit`) | Automatic (livepatch core) |

Both modules target **arm64, kernel 6.18**.

> This is one of two independent demos in this repo. See
> `../livepatch_stack_demo/README.md` for the other, which shows how the
> kernel decides *when* it's safe to switch an in-flight task to a patched
> function.
...
# Or build both modules and the demo binary at once
make
```
...
See `function_pointer/README.md` and `livepatch/README.md` for full build
instructions, expected kernel logs, and module-specific notes.
